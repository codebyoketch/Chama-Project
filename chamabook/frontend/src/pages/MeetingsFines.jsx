...
const printMinutes = (meeting, report) => {
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>Meeting Minutes — ${meeting.title}</title>
    ...
    ${minutesHtml}
    <div class="sig">
      <div><div class="sig-line">Chairperson Signature &amp; Date</div></div>
      <div><div class="sig-line">Secretary Signature &amp; Date</div></div>
    </div>
    </body></html>`)
  win.document.close()
  win.focus()
    ...
    <div>
      ...
      {loading ? <p>Loading...</p> : (
        <>
          ...
          {upcoming.length > 0 && (
            <>
              <h3 style={{ margin: '0 0 14px', fontSize: 13, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Upcoming</h3>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
                          {[
                            [`❌ Absent: KES ${m.fine_absent || 0}`, '#fef2f2', '#dc2626'],
                            [`🙏 Apology: KES ${m.fine_absent_apology || 0}`, '#fef3c7', '#d97706'],
                            [`⏰ Late: KES ${m.fine_late || 0}`, '#fff7ed', '#c2410c'],
                          ].map(([label, bg, color]) => (
                            <span key={label} style={{ fontSize: 11, padding: '2px 10px', borderRadius: 10, background: bg, color, fontWeight: 600 }}>{label}</span>
                          ))}
                        </div>
                      </div>
          ...
          {past.length > 0 && (
            <>
              <h3 style={{ margin: '0 0 14px', fontSize: 13, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Past Meetings</h3>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              ...
              {/* Fine overrides for this meeting */}
              <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, padding: 14 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: '#92400e', marginBottom: 10 }}>⚠️ Fine Amounts — override defaults for this meeting</div>
                ...
                <div style={{ fontSize: 11, color: '#92400e', marginTop: 8, opacity: 0.8 }}>
                  Default values loaded from Settings → Fines. Change here to override for this meeting only.
                </div>
              </div>
      ...
      {showAttendance && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {/* Fine override section */}
            <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, padding: 14, marginBottom: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#92400e', marginBottom: 10 }}>⚠️ Fine Amounts — adjust if needed before saving</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                <div>
            ...
            {(() => {
              ...
              const lateFineCost = c.late * attendanceForm.fine_late
              const absentFineCost = c.absent * attendanceForm.absent_fine
              const apologyFineCost = c.apology * attendanceForm.absent_no_apology_fine
              const totalFineCost = lateFineCost + absentFineCost + apologyFineCost
              return (
                <div style={{ marginBottom: 16 }}>
                  ...
                  {totalFineCost > 0 && (
                    <div style={{ background: '#fef2f2', borderRadius: 8, padding: '7px 14px', fontSize: 12, color: '#dc2626', fontWeight: 600 }}>
                      💰 Fines to be issued: KES {totalFineCost.toLocaleString()}
                      <span style={{ fontWeight: 400, marginLeft: 8, color: '#92400e' }}>
                        ({c.absent} absent × {attendanceForm.absent_fine} + {c.apology} apology × {attendanceForm.absent_no_apology_fine} + {c.late} late × {attendanceForm.fine_late})
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: '#374151', marginBottom: 8 }}>Mark Attendance</div>
            <div style={{ border: '1px solid #e2e8f0', borderRadius: 8, overflow: 'hidden', marginBottom: 20 }}>
              {members.map((m, i) => {
                const status = attendance[m.ID] || 'present'
                const rowBg = status === 'absent' ? '#fff8f8' : status === 'absent_apology' ? '#fffef5' : status === 'late' ? '#fffbf0' : 'white'
                return (
                  <div key={m.ID} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px',
                    borderBottom: i < members.length - 1 ? '1px solid #f1f5f9' : 'none', background: rowBg }}>
                    <div>
                    ...
                    <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                      {[
                        ...
                        <button key={val} onClick={() => setAttendance(a => ({ ...a, [m.ID]: val }))}
                          style={{ padding: '5px 9px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 600,
                            background: status === val ? color : bg,
                            color: status === val ? 'white' : color,
                            outline: status === val ? `2px solid ${color}` : 'none' }}>
                          {icon} {label}
                        </button>
                      ))}
                    </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                ...
                <button onClick={() => setMinutesItems(items => [...items, { title: '', content: '' }])}
              ...
              {minutesItems.map((item, idx) => (
                <div key={idx} style={{ background: '#f8fafc', borderRadius: 8, padding: '12px 14px', marginBottom: 10, border: '1px solid #e2e8f0' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b' }}>Item {idx + 1}</span>
                  ...
                  <input style={{ ...inp, marginBottom: 8 }} placeholder="e.g. Treasurer's Report, Any Other Business..."
                    value={item.title}
                    onChange={e => setMinutesItems(items => items.map((it, i) => i === idx ? { ...it, title: e.target.value } : it))} />
                  <textarea style={{ ...inp, minHeight: 60, resize: 'vertical' }} placeholder="Discussion notes and resolutions..."
                    value={item.content}
                    onChange={e => setMinutesItems(items => items.map((it, i) => i === idx ? { ...it, content: e.target.value } : it))} />
                </div>
              ))}
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 580, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            {report.total_fines > 0 && (
              <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13 }}>
                💰 Total fines issued: <strong>KES {report.total_fines?.toLocaleString()}</strong>
                <div style={{ fontSize: 11, color: '#92400e', marginTop: 4 }}>
                  Absent: KES {report.meeting?.fine_absent} &nbsp;·&nbsp;
                  Apology: KES {report.meeting?.fine_absent_apology} &nbsp;·&nbsp;
                  Late: KES {report.meeting?.fine_late || 0}
                </div>
              </div>
            )}
            ...
            {[
              ...
              <div key={title} style={{ marginBottom: 14 }}>
                ...
                <div style={{ background: bg, borderRadius: 8, padding: '8px 12px' }}>
                  {list.map(a => (
                    <div key={a.ID} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', fontSize: 13, borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                  ...
                  try {
                    ...
                    if (Array.isArray(items) && items.length > 0) {
                      return items.map((item, idx) => (
                        <div key={idx} style={{ background: '#f8fafc', borderRadius: 8, padding: '12px 14px', marginBottom: 8, border: '1px solid #e2e8f0' }}>
